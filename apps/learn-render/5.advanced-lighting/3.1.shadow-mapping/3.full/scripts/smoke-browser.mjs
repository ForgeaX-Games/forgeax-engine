#!/usr/bin/env node
// apps/learn-render/5.advanced-lighting/3.1.shadow-mapping/3.full/scripts/smoke-browser.mjs
// feat-20260621-learn-render-5-3-production-shadow-demos M2 / M2-T-SMOKE-BROWSER.
//
// Playwright e2e smoke for the directional production shadow demo. Spawns a
// local vite dev server on port 5199, drives headed Chrome with WebGPU enabled,
// and asserts:
//   (a) pipelineCount > 0
...
const PKG_NAME = '@forgeax/app-learn-render-5-advanced-lighting-3-1-shadow-mapping-3-full';
...
console.log(
  `\n[smoke-browser] GREEN -- ${captured.pipelines.length} pipelines, ${captured.deviceErrors.length} device errors. ` +
    'Directional shadow demo smoke passed.',
);
process.exit(0);
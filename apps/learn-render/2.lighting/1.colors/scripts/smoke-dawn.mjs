...
console.log(
  `[smoke] oracle=color-light cubeCenter=${JSON.stringify(cubeCenter)} witness=${colorLightWitness} lightColor=${FALSIFY_LIGHT_COLOR || 'white'} lightIntensity=${FALSIFY_LIGHT_INTENSITY || 'default'} intensityWitness=${intensityLightWitness} falsifier=${FALSIFY_NO_LIGHT ? 'no-light' : FALSIFY_LIGHT_COLOR ? 'blue-light' : FALSIFY_LIGHT_INTENSITY ? 'low-intensity' : 'none'}`,
);
...
console.log(
  `[smoke] PASS - 5 criteria GREEN: backend=webgpu, frames=${framesObserved}, meshed sites above threshold=${meshedCount}/${meshSiteNames.length}, oracle=color-light/${FALSIFY_LIGHT_COLOR || 'white'}, lightIntensity=${FALSIFY_LIGHT_INTENSITY || 'default'}, RhiError count=0, wallTotalMs=${wallTotalMs}`,
);

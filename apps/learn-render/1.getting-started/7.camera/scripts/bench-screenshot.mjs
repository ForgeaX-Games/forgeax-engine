#!/usr/bin/env node
// apps/learn-render/1.getting-started/7.camera/scripts/bench-screenshot.mjs
//
// Records the M11 golden PNG for `pnpm bench:pixel-parity` (entry
// `app-learn-render-1-getting-started-7-camera`, plan-strategy section
// 2.7 Open Q-1 option (a) - each milestone records its own baseline).
// Mirrors apps/learn-render/.../6.coordinate-systems/scripts/bench-
// screenshot.mjs structure modulo the LO 1.7 specifics (port 5186 +
// capture hook name `__captureCamera` + output round-7-camera.png).
//
// Pipeline:
//   1. spawn vite preview on port 5186 (vite.config.ts strictPort).
//   2. wait-on tcp 30s.
...
//      screenshots/round-7-camera.png.
//
...
// principle 6). The output is deterministic across runs (the LO 1.7
// chapter spawns one cube + one camera at the origin; without input
// events the camera stays still and the captured frame is stable).
...
const OUT_PATH = resolve(SUBMODULE_DIR, 'round-7-camera.png');
...
async function main() {
  ...
  try {
    preview = spawn(
      ...
      [
        '--filter',
        '@forgeax/app-learn-render-1-getting-started-7-camera',
        'exec',
        'vite',
        'preview',
        '--port',

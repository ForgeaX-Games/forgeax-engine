...
try {
  ...
  try {
    await page.waitForFunction(() => globalThis.__bevyMesh2dReady === true, null, { timeout: 30000 });
  } catch (error) {
    console.error(`[smoke-browser] ready timeout; diagnostics=${errors.join(' | ') || 'none'}`);
    throw error;
  }
  await page.screenshot({ path: 'artifacts/mesh2d-browser.png' });
  ...
  console.log('[smoke-browser] PASS - Chrome loaded mesh2d and reached the running-system ready marker');
  await browser.close();
} finally {
  vite.kill('SIGTERM');
  await delay(100);

...
let coloredPixels = 0;
for (let i = 0; i < tight.length; i += 4) if (Math.max(tight[i], tight[i + 1], tight[i + 2]) - Math.min(tight[i], tight[i + 1], tight[i + 2]) > 35 && Math.max(tight[i], tight[i + 1], tight[i + 2]) > 40) coloredPixels += 1;

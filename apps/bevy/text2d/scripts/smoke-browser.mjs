...
const artifactDir = resolve(process.env.SMOKE_ARTIFACT_DIR ?? resolve(new URL('../artifacts/', import.meta.url).pathname));
...
const errors = [];
try {
  vite = spawn('pnpm', ['-F', '@forgeax/bevy-text2d', 'dev', '--', '--host', '127.0.0.1', '--port', '0'], {
    cwd: root,
  ...
  await page.waitForFunction(() => globalThis.__bevyText2dReady === true, undefined, { timeout: 30_000 });
  await sleep(2_000);
  await page.screenshot({ path: resolve(artifactDir, 'browser.png') });
  if (errors.length) throw new Error(errors.join('\n'));
  console.log(`[smoke] PASS - browser text2d rendered; screenshot=${resolve(artifactDir, 'browser.png')}`);
} finally {
  await browser?.close();
  stopping = true;

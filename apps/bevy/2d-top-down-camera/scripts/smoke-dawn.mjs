#!/usr/bin/env node
// bevy-2d-top-down-camera headless dawn smoke — proves Bevy
// 2d_top_down_camera behavior: WASD player movement + orthographic camera
// smooth-tracking via smoothDamp. Shared scene from src/top-down-camera.ts.
...
const { buildTopDownWorld, cameraPosition, stepTopDownCamera } = await import(resolve(here, '..', 'src', 'top-down-camera.ts'));
...
console.log(`[bevy-2d-top-down-camera] backend=${renderer.backend}`);
...
// --- drive with W + D held to move player diagonally ---
...
function wdSnapshot() {
  return {
    keyboard: { down: (k) => k === 'w' || k === 'd' || k === 'W' || k === 'D', up: (k) => k !== 'w' && k !== 'd' && k !== 'W' && k !== 'D', pressed: (k) => k === 'w' || k === 'd' || k === 'W' || k === 'D', released: (k) => k !== 'w' && k !== 'd' && k !== 'W' && k !== 'D' },
    mouse: { movementDelta: { x: 0, y: 0 }, wheelDelta: 0, button: () => false },
    gamepad: { button: () => false, axis: () => 0 },
    timestamp: 0,
  };
...
for (let i = 0; i < SMOKE_MIN_FRAMES; i++) {
  ...
  // W + D held for the entire run — diagonal movement (up-right).
  stepTopDownCamera(world, FIXED_DT, wdSnapshot());

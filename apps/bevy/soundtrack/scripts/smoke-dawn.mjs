#!/usr/bin/env node
// Headless structural smoke for soundtrack.
...
const { buildSoundtrackWorld } = await import('../src/soundtrack.ts');
...
const scene = buildSoundtrackWorld(app.world, WIDTH / HEIGHT);
const errors = [];
...
console.log(`[soundtrack] backend=${app.renderer.backend} frames=${frames} audioResource=${hasAudioEngine} camera=${scene.camera} peaceful=${scene.peaceful} battle=${scene.battle}`);
const failures = [];

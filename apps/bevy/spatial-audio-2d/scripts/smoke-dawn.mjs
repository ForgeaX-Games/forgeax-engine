#!/usr/bin/env node
// Headless structural smoke for spatial_audio_2d.
// Dawn has no AudioContext, so this gate checks the real createApp audio
// resource, scene construction, renderer draws, and absence of RHI errors.
...
const canvas = {
  ...
  getContext(kind) {
    ...
    return {
      ...
      unconfigure() {},
      getCurrentTexture() {
        if (!target) ensureTarget(sharedDevice, 'rgba8unorm');
        return target;
      },
    };
  },
...
const scene = buildSpatialAudio2dWorld(app.world, WIDTH / HEIGHT);
const errors = [];
...
console.log(`[spatial-audio-2d] backend=${app.renderer.backend} frames=${frames} audioResource=${hasAudioEngine} camera=${scene.camera} listener=${scene.listener} emitter=${scene.emitter}`);
...
console.log('[smoke] PASS - structural audio resource, scene, 300 draws, and RHI error gates are green.');
sharedDevice?.destroy?.();

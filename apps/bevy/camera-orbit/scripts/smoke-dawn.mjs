// bevy-camera-orbit headless dawn smoke — proves Bevy camera_orbit's spatial
// contract: deterministic yaw/pitch moves a target-facing camera around a static
// cube-on-plane scene while preserving a fixed radius. Browser app and smoke import
// the same src/camera-orbit.ts builder and step.
//
...
const { buildCameraOrbitWorld, cameraRadius, OrbitCamera, ORBIT_DISTANCE, stepCameraOrbit } = await import(resolve(here, '..', 'src', 'camera-orbit.ts'));
...
console.log(`[bevy-camera-orbit] backend=${renderer.backend}`);
...
let earlyFrame;
let lateFrame;
let earlyRadius = Number.NaN;
let earlyCameraPos;
let finalCameraPos;
let earlyRoll = Number.NaN;
let finalRoll = Number.NaN;
for (let i = 0; i < SMOKE_MIN_FRAMES; i++) {
  ...
  stepCameraOrbit(world, FIXED_DT, {
    movementDelta: { x: 2, y: 1 },
    leftPressed: i >= Math.floor(SMOKE_MIN_FRAMES * 0.2) && i < Math.floor(SMOKE_MIN_FRAMES * 0.3),
    rightPressed: i >= Math.floor(SMOKE_MIN_FRAMES * 0.7) && i < Math.floor(SMOKE_MIN_FRAMES * 0.78),
  });
...
// The cube-on-plane scene remains visible near the center of the orbit.
...
console.log(`[smoke] cameraRadius=${finalRadius.toFixed(4)} cameraPos=${JSON.stringify(finalCameraPos)}`);

// --- dump both PNGs so the loop eyeballs camera-orbit motion + compares to Bevy ---
try {
...
if (dist <= SMOKE_PIXEL_THRESHOLD) {
  failures.push(`(c) NDC-center ${JSON.stringify(ndcCenter)} too close to black (dist ${dist.toFixed(4)} <= ${SMOKE_PIXEL_THRESHOLD}) — scene not visible`);
}
if (motionMeanDelta <= MOTION_THRESHOLD) {
  failures.push(`(d) motionMeanDelta ${motionMeanDelta.toFixed(5)} <= ${MOTION_THRESHOLD} — camera did NOT visibly orbit`);
}
const radiusStable = Math.abs(earlyRadius - ORBIT_DISTANCE) < 1e-4 && Math.abs(finalRadius - ORBIT_DISTANCE) < 1e-4;
const positionChanged = earlyCameraPos?.some((value, index) => Math.abs(value - (finalCameraPos?.[index] ?? value)) > 1e-3) ?? false;
const rollChanged = Math.abs(finalRoll - earlyRoll) > 1e-3;
if (!radiusStable || !positionChanged || !rollChanged) {
  failures.push(`(e) camera orbit invalid: earlyRadius=${earlyRadius} finalRadius=${finalRadius} positionChanged=${positionChanged} rollChanged=${rollChanged}`);
}
...
console.log(`[smoke] PASS - 6 criteria GREEN: backend=webgpu, frames=${framesObserved}, sceneVisible=${dist.toFixed(4)}, motionMeanDelta=${motionMeanDelta.toFixed(5)}, fixedRadius+orbit OK, RhiError count=0`);
device.destroy?.();

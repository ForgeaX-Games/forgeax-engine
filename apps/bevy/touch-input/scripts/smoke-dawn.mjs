...
const touch = (pointerId, x, y, phase, pressure = 0.5) => ({
  pointerId,
  phase,
  x,
  y,
...
const contact = (pointerId, x, y, active, pressure = 0.5) => ({
  pointerId,
  x,
  y,
  pressure,
...
const phases = [
  { pointerEvents: [touch(9, 80, 60, 'down')], pointers: [contact(9, 80, 60, true)] },
  { pointerEvents: [touch(9, 140, 90, 'move')], pointers: [contact(9, 140, 90, true)] },
  { pointerEvents: [touch(9, 180, 110, 'up')], pointers: [] },
  { pointerEvents: [touch(10, 220, 120, 'down')], pointers: [contact(10, 220, 120, true)] },
  { pointerEvents: [touch(10, 220, 120, 'cancel')], pointers: [] },
...
const input = {
  sample() {
    ...
    return {
      ...
      focused: true,
      pointerLocked: false,
      pointers: phase.pointers,
      pointerEvents: phase.pointerEvents,
...
const { buildTouchInputWorld, readTouchInputState, stepTouchInput } = await import(resolve(appRoot, 'src', 'touch-input.ts'));
const appResult = await createApp({
  ...
  addEventListener() {},
  removeEventListener() {},
}, { input }, { shaderManifestUrl: manifestUrl });
...
buildTouchInputWorld(app.world);
app.world.addSystem(Update, {
  name: 'bevy-touch-input-read',
  after: [FRAME_START_SCAN_SYSTEM_NAME],
  queries: [],
  fn: (world) => stepTouchInput(world, world.getResource(INPUT_SNAPSHOT_RESOURCE_KEY)),
});
...
const state = readTouchInputState(app.world);
app.stop();
...
console.log(`[smoke] frames=${frames} visiblePixels=${visiblePixels} pressed=${state.justPressedTouches} released=${state.justReleasedTouches} canceled=${state.canceledTouches} justPressedActive=${state.justPressedActiveTouches} activeFrames=${state.activeContactFrames} lastActive=(${state.lastActiveX},${state.lastActiveY}) errors=${errors.length} png=${pngOut}`);
const failures = [];
...
if (state.justPressedTouches !== 2 || state.justReleasedTouches !== 1 || state.canceledTouches !== 1) failures.push(`touch edges=${JSON.stringify(state)}`);
if (state.justPressedActiveTouches !== 2 || state.activeContactFrames < 2) failures.push(`active=${JSON.stringify(state)}`);
if (state.lastActiveX !== 220 || state.lastActiveY !== 120) failures.push(`last active=${JSON.stringify(state)}`);
...
console.log('[smoke] PASS - touch down, release, cancel, and active contact state were proven at the frozen frame boundary');

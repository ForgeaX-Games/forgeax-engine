#!/usr/bin/env node
// Chrome/WebGPU smoke for the real browser SceneAsset update path.
...
const artifactDir = resolve(root, 'apps', 'bevy', 'update-gltf-scene', 'artifacts');
mkdirSync(artifactDir, { recursive: true });
const vite = spawn('pnpm', ['-F', '@forgeax/bevy-update-gltf-scene', 'dev'], { cwd: root, stdio: ['ignore', 'pipe', 'pipe'] });
let url;
...
try {
  ...
  try {
    ...
    await page.waitForFunction(() => window.__bevyUpdateGltfSceneReady === true, undefined, { timeout: 20_000 });
    ...
    const screenshotPath = resolve(artifactDir, 'update-gltf-scene-browser.png');
    ...
    console.log('[smoke-browser] PASS - Chrome updated instantiated glTF descendants through the app loop');
  } finally {
    await browser.close();
  }
} catch (error) {

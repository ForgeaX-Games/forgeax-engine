#!/usr/bin/env node
// bevy-shadow-biases structural dawn smoke (a) backend=webgpu (b) frames >= MIN (c) RhiError=0
...
console.log(`[shadow-biases] backend=${r.backend}`);const errs=[];r.onError(e=>errs.push(e));
if(!(await r.ready).ok){console.error('[smoke] FAIL ready');process.exit(1)}
const cube=createBoxGeometry(1,1,1,1,1,1);if(!cube.ok)process.exit(1);const sphere=createSphereGeometry(0.3,16,8);if(!sphere.ok)process.exit(1);
const cH=w.allocSharedRef('MeshAsset',cube.value);const sH=w.allocSharedRef('MeshAsset',sphere.value);
const mat=w.allocSharedRef('MaterialAsset',Materials.standard({baseColor:[0.8,0.8,0.8,1],metallic:0,roughness:0.5}));
w.spawn({component:Transform,data:{pos:[0,-1.5,0],quat:[0,0,0,1],scale:[20,0.02,20]}},{component:MeshFilter,data:{assetHandle:cH}},{component:MeshRenderer,data:{materials:[mat]}});
for(let x=-5;x<=5;x+=2)w.spawn({component:Transform,data:{pos:[x*0.5,0.5,0],quat:[0,0,0,1],scale:[1,1,1]}},{component:MeshFilter,data:{assetHandle:sH}},{component:MeshRenderer,data:{materials:[mat]}});
w.spawn({component:DirectionalLight,data:{direction:[-0.4,-0.8,-0.5],color:[1,1,1],intensity:2,depthBias:0.005,normalBias:0.05}});

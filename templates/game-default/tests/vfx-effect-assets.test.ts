...
describe('game-default authored VFX effects', () => {
  it('keeps hit and charge effects as separate GUID-addressed Pack assets', () => {
    ...
    expect(hit.effect.emitters.map((emitter) => emitter.output.kind)).toEqual(['billboard', 'mesh']);
    expect(charge.effect.emitters.map((emitter) => emitter.output.kind)).toEqual(['billboard', 'mesh']);
  });

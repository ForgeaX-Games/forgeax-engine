...
export interface HudHandle {
  ...
  setTargetProfileActive(active: boolean): void;
...
function failedHud(error: UiError): HudHandle {
  return { error, setScore() {}, setTargetProfileActive() {}, setMode() {}, setLockStatus() {}, floatScore() {}, dispose() {} };
}
...
export function installHud(opts: {
  ...
  const applyMission = (): void => {
    if (!mission) return;
    mission.textContent = currentScore < 50
      ? `Mission 1/2 · Score 50 · ${currentScore}/50`
      : targetProfileActive
        ? 'Mission complete · Target profile applied · R to replay'
        : 'Mission 2/2 · Press P to apply the authored target profile';
    mission.dataset.complete = currentScore >= 50 && targetProfileActive ? 'true' : 'false';
  };
  const applyMode = (mode: ViewMode): void => {
    ...
    if (hint) hint.textContent = mode === 'fps'
      ? 'WASD move · click/F shoot · ESC releases · R restarts'
      : mode === 'orbit'
        ? 'WASD move · drag to orbit · click/F shoot · R restarts'
        : mode === 'pan'
          ? 'Arrows pan · wheel zoom · click/F shoot · R restarts'
          : 'WASD move · aim and click to shoot · R restarts';
  ...
  const setTargetProfileActive = (active: boolean): void => {
  ...
  return { setScore, setTargetProfileActive, setMode: applyMode, setLockStatus, floatScore, dispose: instance.dispose };
}

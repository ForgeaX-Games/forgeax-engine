...
const EMPTY_VIDEO_TEXTURE = { available: false, active: 'original', swaps: 0, guid: null, name: null, kind: null, url: null } as const;
const EMPTY_MULTI_WORLD = { enabled: false, worldCount: 1, entityCount: 0, cameraOwner: 0, resourceOwner: 0 } as const;
const EMPTY_WORLD_SCORE_TEXT = { available: false, baked: false, active: false, text: '', age: 0, position: [0, 0, 0], fontSource: 'legacy-pack', fontGuid: null, toggles: 0 } as const;
...
export function installGameplayProjection(args: GameplayProjectionContext): void {
  ...
  const projectionDisposers = [
    ...
    projection.registerAction({
      ...
      run: () => asProjection(args.toggleProfile()),
    }),
    projection.registerAction({

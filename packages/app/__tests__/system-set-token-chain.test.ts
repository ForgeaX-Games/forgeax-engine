...
describe('SystemSet root-entry token chain', () => {
  it('configures tokens imported from every owner root entry', () => {
    ...
    expect(world.configureSets({ set: TransformSet, before: [AnimationSet] }).ok).toBe(true);
    expect(world.configureSets({ set: InputSet, before: [StateSet] }).ok).toBe(true);
    expect(world.configureSets({ set: PhysicsSet, after: [StateSet] }).ok).toBe(true);
  });

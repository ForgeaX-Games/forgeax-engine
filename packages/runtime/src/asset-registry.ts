...
export class AssetRegistry {
   ...
   * scene itself" registry entry. Shares the GUID-resolution + shared-ref +
   * SceneAssetResolver prelude with `instantiate`, but calls
   * `world.instantiateSceneFlat` instead of `world.instantiateScene`: NO
   * synthetic SceneInstance root, NO forced `ChildOf` on top-level members. The
   * scene's own entities become plain top-level world entities; nested prefabs
   * (`mounts[]`) still become their own SceneInstance anchors. Returns the set
   * of top-level entity handles.
   *
   * Use this to OPEN a scene for authoring; use `instantiate` (anchor) at
  ...
  instantiateFlat<T extends SceneAsset>(
    ...
    // Post-spawn Skin.joints wiring, per top-level root (each GLB root keeps its
    // own ChildOf subtree, so joint resolution is scoped to each subtree exactly
    // as the anchor path scopes it to the single synthetic root).
    const self = this;

...
// '@forgeax/engine-console' string never lands in packages/runtime/dist/**
...
// The literal `@forgeax/engine-console` package-name guard below already
...
if (hits.length > 0) {
  ...
  process.stderr.write(
    '[rerun]  node packages/console/scripts/check-console-not-in-engine-bundle.mjs\n',
  );
  process.stderr.write(
    `[hint]   convert static import to await import('@forgeax/engine-console/server') inside Renderer.startConsole(opts) per D-P4 bundle physical isolation; hits:\n`,
  );

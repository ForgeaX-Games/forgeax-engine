#!/usr/bin/env node
// AC-09 grep gate: assert `@forgeax/engine-console` package.json#dependencies
...
const PKG_PATH = join(process.cwd(), 'packages', 'console', 'package.json');
...
function main() {
  ...
  if (hits.length > 0) {
    process.stderr.write(
      `[fail] AC-09: forbidden CLI argparse dep(s) in @forgeax/engine-console package.json#dependencies:\n`,
    );
  ...
  process.stdout.write(
    `[ok] AC-09: 0 forbidden CLI argparse deps in @forgeax/engine-console package.json\n`,
  );
}

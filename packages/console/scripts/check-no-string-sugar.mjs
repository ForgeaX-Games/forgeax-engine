...
// helpers are decommissioned from `packages/console/src/**`.
//
...
// helper - both shipped from `defineSugar.ts`. Hits in `packages/console/src/**`
...
// `packages/console/src/**` recursively; .ts / .mjs / .js / .d.ts all
...
const ROOT = join(process.cwd(), 'packages', 'console', 'src');
...
function main() {
  ...
  if (totalHits > 0) {
    process.stderr.write(
      `[fail] AC-02: legacy buildXxxScript identifier hits in packages/console/src/**:\n`,
    );
    for (const h of hits) {
      process.stderr.write(`  ${h.file}:${h.line}  ${h.symbol}\n`);
    }
    process.stderr.write(
      `\n  expected: 0 hits across packages/console/src/**\n` +
  ...
  process.stdout.write(
    `[ok] AC-02: 0 hits for legacy buildXxxScript identifiers in packages/console/src/**\n`,
  );
}

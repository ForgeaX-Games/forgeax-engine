...
// string-array helpers are decommissioned from `packages/console/src/cli.ts`.
//
...
const CLI_PATH = join(process.cwd(), 'packages', 'console', 'src', 'cli.ts');
...
function main() {
  ...
  if (hits.length > 0) {
    process.stderr.write(
      `[fail] AC-08: legacy buildHelpText / buildInspectHelpText function definition hits in packages/console/src/cli.ts:\n`,
    );
    for (const h of hits) {
      process.stderr.write(`  ${CLI_PATH}:${h.line}  ${h.content}\n`);
    }
    process.stderr.write(
      `\n  expected: 0 hits in packages/console/src/cli.ts production source\n` +

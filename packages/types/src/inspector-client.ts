...
// `@forgeax/engine-remote/src/cli.ts:367-451` so the engine-console base
...
// - `client.eval(script)` — single-entry RPC for cli-ecs IIFE script
...
// - types is already the wire-protocol shape SSOT (RemoteErrorCode 6
...
import type { RemoteError, RemoteErrorCode } from './index';
...
export interface InspectorClient {
   ...
   * {@link RemoteError} 4-field surface — AI users branch on `.code`.
   */
 ...
 * (`@forgeax/engine-remote` base CLI + `@forgeax/engine-ecs` cli-ecs
 ...
 * {@link RemoteError} whose `.code` is one of the 4 closed members
    ...
    ws.on('message', (raw: WebSocket.RawData) => {
      ...
      if (parsed.error) {
        const data = parsed.error.data as
          | { code?: RemoteErrorCode; expected?: string; hint?: string }
          | undefined;
        if (data?.code && data.expected && data.hint) {
          slot.reject(
            Object.assign(
              new Error(
                `[RemoteError ${data.code}] expected: ${data.expected}; hint: ${data.hint}`,
              ),
              {
                name: 'RemoteError',
                code: data.code,

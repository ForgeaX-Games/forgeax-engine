...
//   intercepting `/shaders/manifest.json` + lazy `transformRequest`
...
/**
 * Manifest URL emitted by `generateBundle` (line ~1460) and served by the
 * dev `configureServer` middleware. Single SSOT used both by emit + by the
 * `forgeaxBundlerAdapter` virtual-module source so the AI user never types
 * the literal in app code (AC-12; plan-strategy D-4 q7-A SSOT).
 */
 ...
 *
 * Why a JSON.stringify literal for the URL: keeps escaping correct if the
 * URL ever needs to carry special characters; today it is a simple ASCII
 * path but the call site is future-safe.
 */
...
export function forgeaxBundlerAdapter() {
  return {
    shaderManifestUrl: ${JSON.stringify(SHADER_MANIFEST_URL)},
 ...
 * | `configureServer(server)` | dev-only: middleware serves `/shaders/manifest.json` from the same `state.entries` Map (II-A from plan-strategy §2 D-P2) |
 */
export function forgeaxShader(options: ForgeaXShaderOptions = {}): ForgeaXShaderPlugin {
  ...
  return {
    ...
    generateBundle(this: MinimalPluginContext): void {
      ...
      this.emitFile({
        type: 'asset',
        fileName: 'shaders/manifest.json',
    ...
    //   the example app fetches `/shaders/manifest.json` over HTTP, but no one
    ...
    // - Filter req.url === '/shaders/manifest.json'; on miss call next() with no
    ...
    //   empty manifest (II-5).
    configureServer(server: ViteDevServerLike): void {
      const manifestUrl = '/shaders/manifest.json';

  ...
  device.lost
    .then((info) => {
      ...
      if (safe.reason !== 'destroyed') {
        // Keep recorder generations aligned with recoverable device loss only.
        // `destroyed` is the WebGPU teardown signal and the renderer remains
        // intentionally `alive` for that path; resetting a capture there
        // would discard the only device reference while leaving the host with
        // no recovery transition to drive.
...
async function makeWebGPURenderer(
  ...
  let recoverInFlight: Promise<Result<void, RecoverFailure>> | undefined;
  ...
  // same instance.
  let shaderInstance: ShaderCatalog | null = null;
  // feat-20260523-shader-template-instance-split M9-T03 (D-PipelineBuilder):
  // the ShaderCatalogDevice adapter is created once and shared between the
  // ShaderCatalog instance (manifest hash -> ShaderModule) AND the M9
  ...
  const getShader = (): ShaderCatalog => {
    if (shaderInstance === null) {
      ...
      shaderInstance = new ShaderCatalog({
  ...
  let viewAndSceneDepthMaterialPipelineLayout: PipelineLayout | null = null;
  ...
  const getOrBuildViewAndSceneDepthMaterialPipelineLayout = (): PipelineLayout | null => {
    if (viewAndSceneDepthMaterialPipelineLayout !== null) {
      return viewAndSceneDepthMaterialPipelineLayout;
    }
    ...
    viewAndSceneDepthMaterialPipelineLayout = plRes.value;
    return viewAndSceneDepthMaterialPipelineLayout;
  };
  ...
  const getOrBuildPerShaderMaterialLayout = (
    ...
    const bglRes = internals.device.createBindGroupLayout(desc);
    if (!bglRes.ok) {
      internals.errorRegistry.fire(bglRes.error);
      return null;
    }
  ...
  const getMaterialShaderPipeline = (
    ...
    const ldrColorFormat: GPUTextureFormat =
      colorFormatOverride ??
      (pipelineState !== null ? pipelineState.colorAttachmentFormat : 'bgra8unorm-srgb');
  ...
  const renderer: RendererAssemblyImplementation = {
    ...
    inspect(): RenderInspection {
      ...
      return Object.freeze({
        ...
        perFramePassNames: Object.freeze([...renderSystem.perFramePassNames]),
    ...
    draw(
      ...
      const renderReadLeases: readonly RenderReadLease[] | undefined =
        frameRequest !== undefined
          ? frameRequest.leases
    ...
    dispose(): RenderResult<void, RenderError> {
      ...
      for (const lease of attachedLeases.keys()) {
        try {
          ...
          const error = wrapDisposeError(cause, 'renderReadLease.dispose');
          cleanupFailures.push(error);
          internals.errorRegistry.fire(error);
        }
      }
    ...
    recover(): Promise<Result<void, RecoverFailure>> {
      ...
      recoverInFlight = (async () => {
        ...
        // disposed renderer or non-device-lost state short-circuits before a
        // GPU work.
        if (disposed) {
        ...
        // (A-AC-12) — only the GpuResidencyCache + canvas context are torn down.
        gpuStore.destroyAll();
...
export function exposeRenderer(renderer: RendererHostImplementation): Renderer {
  ...
  let currentState: RendererState = renderer.inspect().state;
  let disposed = currentState === 'disposed';
...
function structuredRendererCause(cause: unknown, operation: string): RendererOperationCause {
...
function validateRenderProfile(profile: RenderProfile): string | undefined {
...
function freezeRenderProfile(profile: RenderProfile): RenderProfile {
  return Object.freeze({
    ...profile,
 ...
 * ShaderCatalog).
 */
interface ShaderDeviceAdapterInternal extends ShaderCatalogDevice {
  /**
...
// ─── D-S3 Renderer.initialization three-step serial pipeline ─────────────────────────
...
// from `selectSwapChainFormat(storageBufferCapable)` (owned by render-system) into
...
//
// Three-step protection in the render-system `selectSwapChainFormat` owner:
...
//      error registry so AI users can subscribe to Renderer error events
...
// feat-20260601-device/gpu-residency-extraction M1 (D-9 sub-contract 1): texture
// formats prewarmed into the mipmap pipeline cache at `renderer.initialization` so the
...
function requireMeshSsboBuffer(
 ...
 * renderer.initialization` and consume `.code` / `.expected` / `.hint` regardless of

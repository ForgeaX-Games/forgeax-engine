...
async function makeWebGPURenderer(internals: WebGPURendererInternals): Promise<Renderer> {
  ...
  const assets = createAssetRegistry(shaderRegistry, internals.importTransport);
  ...
  const buildAndCachePipeline = (
    ...
    const ctx = buildPipelineContext(variantSet, materialShaderId, meshAttributes, vertexLayout);

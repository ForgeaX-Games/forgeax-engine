...
description: >-
  forgeax-engine 渲染管线与后处理：Camera 内建后处理开关 + skybox + 后处理读相机深度 + 声明式
  RenderGraph 替换整条 per-frame 管线。Use when toggling tonemap/bloom/fxaa/
  msaa/skybox, reading camera depth in a post-process pass ({key, sampleType:'depth'}),
  or writing a custom RenderPipeline.
---

# forgeax-engine-render-pipeline

> 基线: [`5c8c90f1`](../../commit/5c8c90f1) (2026-06-03) · 同步至: feat-20260702-postprocess-camera-depth-read M6
...
> feat-20260702-postprocess-camera-depth-read M4

`reads` 除了 `string[]`（向后兼容的色彩输入），现支持结构化形态 `{ key, sampleType }`。当 `sampleType` 为 `'depth'` 时，引擎在 dispatcher 内透明处理：resolve 深度独占视图 (`aspect: 'depth-only'`)、确保目标纹理有 `TEXTURE_BINDING` 用法、绑定 non-filtering sampler（nearest + clamp-to-edge，非 comparison——后处理 shader 读原始深度值而非 PCF 查找）。

> [!NOTE]
> 承载 `sampleType:'depth'` 的是 **`register` 的 `entry.reads`**（`PostProcessShaderEntry.reads`，SSOT）——dispatcher 从这里认深度。`addFullscreenPass` 的 `opts.reads` 是另一个字段（graph 拓扑边），二者不要混。声明 depth read 的 shader **不必**再传 `params`——引擎自动分配一个最小 UBO 填满 `params@2` 槽（该 BGL 恒含 params 槽）。

WGSL 侧：深度纹理声明为 `texture_depth_2d`，用 `textureSample`（读原始深度值，配 non-filtering sampler；`texture_depth_2d` 上 `textureSampleLevel` 亦合法——见 `packages/shader/src/hdrp-ssao.wgsl`——但**不要**用 `textureSampleCompare`，那是 PCF 阴影比较、需 comparison sampler）。BGL 自动升为 5-entry `fullscreen-post-with-scene-depth`：`@group(1) @binding(0) color@0 + sampler@1 + params@2（uniform，始终存在）+ depthTex@3（texture_depth_2d）+ depthSampler@4（non-filtering）`。

**两步注册**：

```ts
// Step 1: register with structured reads — depth entry declares sampleType:'depth'
renderer.postProcess.register('mypkg::overlay', {
  source: overlayWgsl,
  reads: [
    { key: 'sceneColor' },
...
// Step 2: install pipeline via URP postEffects (AUGMENT, not REPLACE)
renderer.installPipeline({
  kind: 'render-pipeline',
  pipelineId: URP_PIPELINE_ID,
  config: { postEffects: ['mypkg::overlay'] },
});
```
...
fn fs_main(in : VOut) -> @location(0) vec4<f32> {
  ...
  // Convert NDC [0,1] to linear view-space depth, bucket into bands, etc.
  // ...
}
```

> [!IMPORTANT]
> `reads` 中 `sampleType:'depth'` 的 key 必须在 graph 中声明为 color target 且带 `TEXTURE_BINDING` usage。否则 dispatcher 抛 `PostProcessError({ code: 'fullscreen-input-not-found' })`（fail-fast），`.detail.readsKey` / `.detail.passName` 指明无法解析的 key 与 pass 名。`.hint` 包含 `TEXTURE_BINDING` 指引与 pipeline 切换建议。

**Worked example**：`apps/learn-render/5.advanced-lighting/3.3.csm/src/cascade-overlay.{wgsl,ts}`——CSM cascade debug overlay（首个真实消费者），读场景深度按 PSSM split band 着色。
...
| `renderer.postProcess.register(id, entry)` | runtime | `(string, PostProcessShaderEntry) => void` | 登记 fullscreen 后处理 WGSL；重名 throw `PostProcessError`；`entry.reads` 现支持 `{ key, sampleType }` 结构化形态——`sampleType:'depth'` 声明读相机深度 |
...
//    params UBO at @group(1) @binding(2). BGL varies by what you declare:
//      - reads: undefined          -> 2-entry 'fullscreen-post'       (texture@0+sampler@1)
//      - reads: ['colorOnly']      -> 2-entry or 3-entry with params  (同上,params@2 可选)
//      - reads: [{key, sampleType:'depth'}, ...] -> 5-entry 'fullscreen-post-with-scene-depth'
//        (color@0+sampler@1+params@2+depthTex@3+depthSampler@4). params@2 始终存在。
//    register eager-creates the per-id UBO (byteSize >= 16, init = defaultValue);
//    byteSize < 16 or defaultValue.length !== byteSize throws
//    PostProcessError{code:'params-size-mismatch'} at register (fail-fast).
//    depth reads: engine transparently resolves depth-only view, binds non-filtering
//    sampler (nearest+clamp-to-edge, NOT comparison), fail-fast on unresolvable key
//    (PostProcessError{code:'fullscreen-input-not-found'}).
renderer.postProcess.register('mypkg::vignette', {
  source: vignetteWGSL,
  params: { byteSize: 16, defaultValue: new Uint8Array(16) },
  reads: ['hdrComposited'], // or [{ key: 'depth', sampleType: 'depth' }, { key: 'hdrColor' }]
});
...
- **深度不可读（`fullscreen-input-not-found`）**：`reads` 中 `sampleType:'depth'` 的 key 必须在 graph 中声明为 color target 且带 `TEXTURE_BINDING` usage（`0x04`）。dispatcher 在 resolve 深度独占视图前 fail-fast：`PostProcessError({ code: 'fullscreen-input-not-found' })`。`.detail.readsKey` / `.detail.passName` 指明哪个 key 解析失败（typo / 未声明 / usage 缺 `TEXTURE_BINDING`）。`.hint` 含 `TEXTURE_BINDING` 指引与 pipeline 切换建议。WGSL 侧深度纹理用 `textureSample` 读原始深度值（`textureSampleLevel` 同样合法，见 SSAO；避免 `textureSampleCompare`——那需 comparison sampler、做 PCF 比较而非读值）。
...
- 自定义 fullscreen pass 注册与 lookup：`renderer.postProcess.register` + `buildFullscreenPostProcessPass` / `createFullscreenBindGroup` 见 `packages/runtime/src/fullscreen-post-process-pass.ts`；`PostProcessReadEntry` + `PostProcessReadSampleType` 结构化 reads 类型、`BglKind` `'fullscreen-post-with-scene-depth'` 5-entry BGL 见同文件 + `packages/runtime/src/pipeline-spec.ts`
- **Worked example (深度读)**: `apps/learn-render/5.advanced-lighting/3.3.csm/src/cascade-overlay.{wgsl,ts}` -- CSM cascade debug overlay，首个 `sampleType:'depth'` 真实消费者（读场景深度、按 PSSM split band 着色、URP config.postEffects 叠加）